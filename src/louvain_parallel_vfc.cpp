...
#include "louvain_seq.h"    // fallback when numThreads <= 1
#include "graph.h"
#include "hierarchy.h"
#include "core_type.h"
...
// -----------------------------------------------------------------------------
// 1) high-quality, balanced distance-1 coloring (isolate sets)
// -----------------------------------------------------------------------------
...
Graph applyVertexFollowing(const Graph &g, std::vector<int> &mapping) {
    ...
    // if no single-degree vertices, return original
    if (!std::any_of(is_single.begin(), is_single.end(), [](bool b){ return b; }))
        return g;
...
// -----------------------------------------------------------------------------
// 3) One local-move phase using isolate sets
// -----------------------------------------------------------------------------
    ...
    for (const auto &set : sets) {
        ...
        for (size_t k = 0; k < set.size(); ++k) {
            ...
            for (auto &kv : W) {
                int c = kv.first;
                if (c == curr) continue;
                double gain = kv.second - g.degree[v] * commTot[c] * inv2m;
...
// -----------------------------------------------------------------------------
// 5) Main driver: dynamic thresholds + P/E core affinity
// -----------------------------------------------------------------------------
void louvainParallelVFC(
    ...
    for (int level = 0;; ++level) {
        std::cerr << "--- Phase " << level
                  << ": n=" << g.n << ", m=" << g.m << " ---\n";
        bool doColor = (g.n > MIN_RECOLOR_VERTS && lastPhaseGain >= TH_COLORED);
        std::vector<std::vector<int>> sets;
        if (doColor) sets = createIsolateSets(g);
        else {
            sets.clear();
            sets.emplace_back(g.n);
        ...
        while (moved && iter < 100) {
            moved = localMovePhaseWithIsolateSets(g, C, sets, th);
            ++iter;
        }
        double Qpost = computeModularity(g, C);
        double Qprev = Qpost - lastPhaseGain;
